...
const SkillTree = ({ 
  ...
  draggingId, setDraggingId 
    ...
    const treeData = useMemo(() => {
        // 1. Перевірка на null або порожній об'єкт
        ...
        const build = (id, x, y, parentAngle, depth = 0) => {
            ...
            // --- ОСНОВНЕ ВИПРАВЛЕННЯ: використовуємо 'parent' замість 'parent_id' ---
            const children = Object.entries(skills)
            .filter(([cid, s]) => s.parent === id)
            .map(([cid]) => cid);
            ...
            result[id] = {
            ...skill,
            id, // додаємо id в об'єкт для зручності
            parent_id: skill.parent, // мапимо для сумісності з іншим кодом
            ...
            build(childId, nextX, nextY, currentAngle, depth + 1);
            });
        };

        // --- ПОШУК КОРЕНЯ: шукаємо той, у кого parent === null ---
        const rootId = Object.keys(skills).find(id => skills[id].parent === null);
        
        if (rootId) {
            build(rootId, centerX, startY, isLight ? -90 : 90);
        } else {
            console.error("Не вдалося знайти кореневий елемент (parent: null)");
        }
