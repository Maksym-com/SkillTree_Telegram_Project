...
function App() {
  ...
  return (
    <div style={{ background: '#020617', width: '100vw', height: '100vh', position: 'fixed', inset: 0, overflow: 'hidden', fontFamily: 'sans-serif' }}>
      
      <header style={{ position: 'absolute', top: '20px', left: 0, width: '100%', display: 'flex', justifyContent: 'center', zIndex: 10 }}>
        <div onClick={() => setShowProfilePopup(true)} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(15, 23, 42, 0.6)', padding: '8px 16px', borderRadius: '25px', border: '1px solid rgba(59, 130, 246, 0.3)', backdropFilter: 'blur(10px)', cursor: 'pointer' }}>
          {userAvatar ? <img src={userAvatar} style={{ width: '24px', height: '24px', borderRadius: '50%' }} alt="avatar" /> : <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: '#3b82f6' }} />}
      ...
      <TransformWrapper initialScale={0.6} centerOnInit minScale={0.1} limitToBounds={false}>
        <TransformComponent wrapperStyle={{ width: "100vw", height: "100vh" }}>
          <div style={{ width: "2000px", height: "2000px", position: "relative" }}>
              ...
              {Object.entries(treeData).map(([id, data]) => {
                ...
                return (
                  <path key={`line-${id}`}
                    ...
                    stroke={data.level > 0 ? "#3b82f6" : "#1e293b"} strokeWidth={Math.max(2, 10 - data.depth * 2)} fill="none" style={{ opacity: 0.5 }}
                  />
                );
              })}
            </svg>
            ...
            {Object.entries(treeData).map(([id, data]) => (
              <div key={`node-${id}`} style={{ position: 'absolute', left: data.pos.x, top: data.pos.y, transform: 'translate(-50%, -50%)', zIndex: 5 }}>
                <motion.div 
      ...
      <AnimatePresence>
              ...
              {popupMode === 'menu' ? (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '12px', minHeight: '32px', position: 'relative' }}>
                    {isEditingName ? (
                      <input autoFocus value={editedName} onChange={(e) => setEditedName(e.target.value)} onBlur={handleRename} onKeyDown={(e) => e.key === 'Enter' && handleRename()} style={{ background: '#0f172a', color: '#fff', border: '1px solid #3b82f6', borderRadius: '6px', padding: '2px 10px', textAlign: 'center', fontSize: '18px', fontWeight: 'bold', outline: 'none', width: '80%' }} />
                    ) : (
                      <>
                        <h2 style={{ color: '#fff', fontSize: '18px', margin: 0, fontWeight: 'bold' }}>{skills[selectedSkill]?.name}</h2>
                        <button onClick={() => { setIsEditingName(true); setEditedName(skills[selectedSkill]?.name); }} style={{ marginLeft: '8px', background: 'none', border: 'none', cursor: 'pointer', opacity: 0.5 }}>
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 1 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                        </button>
                      </>
                    )}
                  </div>
                  ...
                  {skills[selectedSkill]?.level < 100 ? (
                    ...
                    <div style={menuButtonStyle("#10b981")}>🏆 MASTERED</div>
                  )}
                  ...
                  <button onClick={() => setShowPopup(false)} style={{ width: '100%', color: '#94a3b8', background: 'none', border: 'none', marginTop: '15px', fontSize: '11px' }}>CANCEL</button>
                </>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                  <h3 style={{ color: '#fff', fontSize: '14px', marginBottom: '16px', textAlign: 'center' }}>NEW SKILL UNDER: <span style={{ color: '#3b82f6' }}>{skills[selectedSkill]?.name}</span></h3>
                  <input ref={inputRef} autoFocus value={newSkillName} onChange={(e) => setNewSkillName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddSkill()} placeholder="Enter name..." style={{ width: '100%', padding: '10px', borderRadius: '10px', background: '#0f172a', color: '#fff', border: '1px solid #334155', marginBottom: '20px' }} />
                  <div style={{ display: 'flex', gap: '10px', width: '100%' }}>
                    ...
                    <button onClick={handleAddSkill} disabled={isSubmitting || !newSkillName.trim()} style={{ flex: 1.5, padding: '10px', borderRadius: '10px', background: '#3b82f6', color: '#fff', fontWeight: 'bold' }}>{isSubmitting ? '...' : 'CREATE'}</button>
                  </div>
                </div>
              )}
            </motion.div>
        ...
        {showProfilePopup && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, background: 'rgba(2, 6, 23, 0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 11000, padding: '20px' }} onClick={() => setShowProfilePopup(false)}>
            <motion.div initial={{ scale: 0.9, y: 50 }} animate={{ scale: 1, y: 0 }} style={{ background: '#1e293b', padding: '30px', borderRadius: '32px', border: '1px solid rgba(59, 130, 246, 0.4)', width: '100%', maxWidth: '320px', textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
              <div style={{ marginBottom: '20px' }}>
                {userAvatar ? <img src={userAvatar} style={{ width: '80px', height: '80px', borderRadius: '50%', border: '3px solid #3b82f6' }} alt="profile" /> : <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: '#3b82f6', margin: '0 auto' }} />}
                <h2 style={{ color: '#fff', marginTop: '15px', fontSize: '20px' }}>{firstName}</h2>
                <p style={{ color: '#64748b', fontSize: '12px' }}>ID: {userId}</p>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <button style={menuButtonStyle(theme === 'dark' ? "#fbbf24" : "#3b82f6")} onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
                  {theme === 'dark' ? '☀️ LIGHT MODE' : '🌙 DARK MODE'}
                </button>
                <button style={menuButtonStyle("#ef4444")} onClick={handleResetTree}>⚠️ RESET TREE</button>
                <button onClick={() => setShowProfilePopup(false)} style={{ marginTop: '10px', color: '#64748b', background: 'none', border: 'none' }}>CLOSE</button>
              </div>
            </motion.div>
          </motion.div>
        )}
